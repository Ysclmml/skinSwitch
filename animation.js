...
(function(newDuilib){
	...
	newDuilib.Spine4AnimationPlayer = (function(){
		function Spine4AnimationPlayer (pathPrefix, parentNode, elementId) {
			...
			this.dpr = 1;
			this.nodes = [];
			this.BUILT_ID = 8888;  // 管理当前的动画id.  每个动画id对应一个APNode对象, 存入nodes数组.
			this._dprAdaptive = false;
		...
		Spine4AnimationPlayer.prototype.createTextureRegion = function (image, name) {
			var page = new spine_4.TextureAtlasPage();
			page.name = name;
			...
			var region = new spine_4.TextureAtlasRegion();
			region.page = page;
			region.rotate = false;
			region.width = page.width;
			region.height = page.height;
		...
		Spine4AnimationPlayer.prototype.hasSpine = function (filename) {
			return this.spine.assets[filename] != undefined;
		};

		Spine4AnimationPlayer.prototype.loadSpine = function (filename, skelType, onload, onerror) {
		...
		Spine4AnimationPlayer.prototype.prepSpine = function (filename, autoLoad) {
			var _this = this;
			var spineAssets = _this.spine.assets;
			if (!spineAssets[filename]) {
				if (autoLoad) {
			...
			if (!skelRawData) {
				...
				// var atlas = new spine_4.TextureAtlas(manager.get(filename + '.atlas'), function(path){
				// 	return manager.get(prefix + path);
				// });

				var atlasLoader = new spine_4.AtlasAttachmentLoader(atlas);
				if (asset.skelType.toLowerCase() === 'json') {
		...
		Spine4AnimationPlayer.prototype.playSpine = function (sprite, position){
			if (self.duicfg && !self.duicfg.gameAnimationEffect) return;
			if (sprite == undefined) return console.error('playSpine: parameter undefined');
			if (typeof sprite == 'string') sprite = { name: sprite };
			if (!this.hasSpine(sprite.name)) return console.error('playSpine: [' + sprite.name + '] 骨骼没有加载');
		...
		Spine4AnimationPlayer.prototype.loopSpine = function (sprite, position) {
			if (typeof sprite == 'string') {
				sprite = {
					name: sprite,
					loop: true,
		...
		Spine4AnimationPlayer.prototype.stopSpine = function (sprite) {
			var nodes = this.nodes;
			var id = sprite.id == undefined ? sprite : sprite.id;
			for (var i = 0; i < nodes.length; i++) {
				sprite = nodes[i];
		...
		Spine4AnimationPlayer.prototype.stopSpineAll = function () {
			var sprite;
			var nodes = this.nodes;
			for (var i = 0; i < nodes.length; i++) {
				sprite = nodes[i];
		...
		Spine4AnimationPlayer.prototype.getSpineActions = function (filename) {
			if (!this.hasSpine(filename)) return console.error('getSpineActions: [' + filename + '] 骨骼没有加载');;
		...
		Spine4AnimationPlayer.prototype.getSpineBounds = function (filename) {
			if (!this.hasSpine(filename)) return console.error('getSpineBounds: [' + filename + '] 骨骼没有加载');;
		...
		Spine4AnimationPlayer.prototype.render = function (timestamp) {
			var canvas = this.canvas;
			var offscreen = this.offscreen;
			var dpr = 1;
			if (this.dprAdaptive) {
			...
			for (var i = 0; i < nodes.length; i++) {
				...
				skeleton.color.a = (sprite.renderOpacity == undefined ? 1 : sprite.renderOpacity);
				state.hideSlots = sprite.hideSlots;
				state.update(delta / 1000 * speed);
				state.apply(skeleton);
				skeleton.updateWorldTransform();
