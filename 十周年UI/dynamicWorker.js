...
onmessage = function (e) {
	...
	switch (data.message) {
		case 'CREATE':
			...
			dynamic = new duilib.AnimationPlayer(data.pathPrefix, 'offscreen', data.canvas);
			dynamic.id = data.id;
			dynamics.push(dynamic);
			break;
		case 'PLAY':
			dynamic = dynamics.getById(data.id);
			if (!dynamic) return;
		...
		case "ACTION":
			dynamic = dynamics.getById(data.id);
			if (!dynamic) return
			apnode = getDynamic(dynamic, data.skinID);
			if (!apnode) return
			...
			let playChuKuangSpine = function (apnode, animation, playNode) {
				...
				console.log(actualPlayNode, 'xxxxxxxxxx')
				setTimeout(() => {
			...
			if (data.action === 'GongJi') {
				...
				if (actionParams.name === apnode.name) {
					...
					if (!animation.duration) {
						...
						if (actionParams.showTime) animation.duration = Math.min(actionParams.showTime, animation.duration)
					}
				...
				if (actionParams.name === apnode.name) {
					...
					if (animation) {
						...
						postMessage({'message': 'action', 'chukuang': false})
					}
				} else {
					if (!dynamic.hasSpine(actionParams.name)) {
						dynamic.loadSpine(actionParams.name, "skel", function () {
							postMessage({'message': 'action', 'chukuang': true})
							playChukuang(actionParams)
						}, errPlaySpine)
					} else {
						postMessage({'message': 'action', 'chukuang': true})
		...
		case "HIDE":
			...
			apnode = getDynamic(d, data.skinID);
		...
		case "POSITION":
			...
			if (data.mode === 'daiji') {
				window.postMessage({x: apnode.x, y: apnode.y, scale: apnode.scale, angle: apnode.angle})
			} else {
				if (apnode.chukuangNode) {
					window.postMessage({x: apnode.chukuangNode.x, y: apnode.chukuangNode.y, scale: apnode.chukuangNode.scale})
				} else {
					// 否则以配置中的pos作为出框的位置
					if (!apnode.gongjiAction) window.postMessage(false)
					let actionParams = apnode.player.gongjiAction
					if (!actionParams) window.postMessage(false)
					window.postMessage({x: actionParams.x, y: actionParams.y, scale: actionParams.scale, })
				}
			}
			break
	}
	...
	if (player.pos) {
		...
		if (teshuType === 'object') {
			...
			// 如果是简单的设置为true, 那么说明当前动皮是静态皮肤, 也想出框, 那么和上面一样, 播放当前待机动作到中央
				...
				if (!gongjiAction.showTime) {gongjiAction.showTime = 2}
			} else {
